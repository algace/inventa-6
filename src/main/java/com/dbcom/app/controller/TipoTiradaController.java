...
import com.dbcom.app.service.TipoTiradaService;
...
@Slf4j
@Controller
public final class TipoTiradaController {

	// Atributos de la vista
	private static final String ATTRIBUTE_TIPO = "tipoTirada";

	// Vistas	
	private static final String VIEW_TIPO = ControllerConstants.MAP_PATH_MENU_TIPOS + ATTRIBUTE_TIPO;
	private static final String VIEW_TIPOS = ControllerConstants.MAP_PATH_MENU_TIPOS + "tiposTirada";
	...
	private final TipoTiradaService tipoTiradaService;
	
	@Autowired
	public TipoTiradaController(TipoTiradaService tipoTiradaService) {
		this.tipoTiradaService = tipoTiradaService;
	}
	...
	public String readAll(final Model model) {
		...
		// Contenido
		model.addAttribute(ControllerConstants.ATTRIBUTE_LISTA, this.tipoTiradaService.readAll());		
	...
	/**
	 * Creamos una tipo de tirada sin persistencia
	 * @param model modelo
	 * @return Vista
	 */
	...
	public String create(final Model model) {
		...
		// Creamos el registro
		model.addAttribute(ATTRIBUTE_TIPO, this.tipoTiradaService.create());
	...
	/**
	 * Persistimos la tipo de tirada pasado como parámetro
	...
	public String save(@Valid @ModelAttribute(ATTRIBUTE_TIPO) final TipoTiradaDto tipoTiradaDto, 
		...
		if (bindingResult.hasErrors()) {
			...
			this.tipoTiradaService.save(tipoTiradaDto);
			vista = ControllerConstants.REDIRECT.concat(MAP_READALL_TIPOS);
			log.info(LoggerConstants.LOG_SAVE, tipoTiradaDto.getId());
		}
	...
	public String read(@PathVariable("id") final Short id, final Model model) {
		...
		// Contenido
		model.addAttribute(ATTRIBUTE_TIPO, this.tipoTiradaService.read(id));
	...
	/**
	 * Preparamos la vista para la actulización del tipo de tirada pasado como parámetro
	 * @param id Identificador
	 * @param model Modelo
	 * @return Vista
	 */
	...
	public String updatePOST(@Valid @ModelAttribute(ATTRIBUTE_TIPO) final TipoTiradaDto tipoTiradaDto, 
			final BindingResult bindingResult, final Model model) {		
		...
		if (bindingResult.hasErrors()) {			
			...
			this.tipoTiradaService.update(tipoTiradaDto);
			vista = ControllerConstants.REDIRECT.concat(MAP_READALL_TIPOS);
			log.info(LoggerConstants.LOG_UPDATE, tipoTiradaDto.getId());			
		}
	...
	/**
	 * Preparamos la vista para la eliminación del tipo de tirada pasado como parámetro
	 * @param id Identificador
	 * @param model Modelo
	 * @return Vista
	 */
	...
	public String deleteGET(@PathVariable("id") final Short id, final Model model) {
		...
		model.addAttribute(ControllerConstants.ATTRIBUTE_POPUP_ELIMINAR_PREGUNTA, 
				MessagesConstants.POPUP_ELIMINAR_TIPO_TIRADA_PREGUNTA);
