...
import com.dbcom.app.service.TipoEmplazamientoService;
...
@Slf4j
@Controller
public final class TipoEmplazamientoController {
	
	// Atributos de la vista
	private static final String ATTRIBUTE_TIPO = "tipoEmplazamiento";

	// Vistas	
	private static final String VIEW_TIPO = ControllerConstants.MAP_PATH_MENU_TIPOS + ATTRIBUTE_TIPO;
	private static final String VIEW_TIPOS = ControllerConstants.MAP_PATH_MENU_TIPOS + "tiposEmplazamiento";	
	...
	private final TipoEmplazamientoService tipoEmplazamientoService;
	
	@Autowired
	public TipoEmplazamientoController(TipoEmplazamientoService tipoEmplazamientoService) {
		this.tipoEmplazamientoService = tipoEmplazamientoService;
	}
	...
	public String readAll(final Model model) {
		...
		// Contenido
		model.addAttribute(ControllerConstants.ATTRIBUTE_LISTA, this.tipoEmplazamientoService.readAll());		
	...
	/**
	 * Creamos una tipo de emplazamiento sin persistencia
	 * @param model modelo
	 * @return Vista
	 */
	...
	public String create(final Model model) {
		...
		// Creamos el registro
		model.addAttribute(ATTRIBUTE_TIPO, this.tipoEmplazamientoService.create());
	...
	/**
	 * Persistimos la tipo de emplazamiento pasado como parámetro
	...
	public String save(@Valid @ModelAttribute(ATTRIBUTE_TIPO) final TipoEmplazamientoDto tipoEmplazamientoDto, 
		...
		if (bindingResult.hasErrors()) {
			...
			this.tipoEmplazamientoService.save(tipoEmplazamientoDto);
			vista = ControllerConstants.REDIRECT.concat(MAP_READALL_TIPOS);
			log.info(LoggerConstants.LOG_SAVE, tipoEmplazamientoDto.getId());
		}
	...
	public String read(@PathVariable("id") final Short id, final Model model) {
		...
		// Contenido
		model.addAttribute(ATTRIBUTE_TIPO, this.tipoEmplazamientoService.read(id));
	...
	/**
	 * Preparamos la vista para la actulización del tipo de emplazamiento pasado como parámetro
	 * @param id Identificador
	 * @param model Modelo
	 * @return Vista
	 */
	...
	public String updatePOST(@Valid @ModelAttribute(ATTRIBUTE_TIPO) final TipoEmplazamientoDto tipoEmplazamientoDto, 
			final BindingResult bindingResult, final Model model) {		
		...
		if (bindingResult.hasErrors()) {			
			...
			this.tipoEmplazamientoService.update(tipoEmplazamientoDto);
			vista = ControllerConstants.REDIRECT.concat(MAP_READALL_TIPOS);
			log.info(LoggerConstants.LOG_UPDATE, tipoEmplazamientoDto.getId());			
		}
	...
	/**
	 * Preparamos la vista para la eliminación del tipo de emplazamiento pasado como parámetro
	 * @param id Identificador
	 * @param model Modelo
	 * @return Vista
	 */
	...
	public String deleteGET(@PathVariable("id") final Short id, final Model model) {
		...
		model.addAttribute(ControllerConstants.ATTRIBUTE_POPUP_ELIMINAR_PREGUNTA, 
				MessagesConstants.POPUP_ELIMINAR_TIPO_EMPLAZAMIENTO_PREGUNTA);
	...
	/**
	 * Eliminación del tipo de emplazamiento pasado como parámetro
	 * @param id Identificador
	 * @return Vista
	 */
	@PostMapping(MAP_DELETE_TIPO + "/{id}")
	public String deletePOST(@PathVariable("id") final Short id) {		
		this.tipoEmplazamientoService.delete(id);					
		log.info(LoggerConstants.LOG_DELETE);		
