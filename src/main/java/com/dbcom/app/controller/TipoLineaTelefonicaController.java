...
import com.dbcom.app.service.TipoLineaTelefonicaService;
...
@Slf4j
@Controller
public final class TipoLineaTelefonicaController {

	// Atributos de la vista
	private static final String ATTRIBUTE_TIPO = "tipoLineaTelefonica";

	// Vistas	
	private static final String VIEW_TIPO = ControllerConstants.MAP_PATH_MENU_TIPOS + ATTRIBUTE_TIPO;
	private static final String VIEW_TIPOS = ControllerConstants.MAP_PATH_MENU_TIPOS + "tiposLineaTelefonica";
	...
	private final TipoLineaTelefonicaService tipoLineaTelefonicaService;
	
	@Autowired
	public TipoLineaTelefonicaController(TipoLineaTelefonicaService tipoLineaTelefonicaService) {
		this.tipoLineaTelefonicaService = tipoLineaTelefonicaService;
	}
	...
	public String readAll(final Model model) {
		...
		// Contenido
		model.addAttribute(ControllerConstants.ATTRIBUTE_LISTA, this.tipoLineaTelefonicaService.readAll());		
	...
	/**
	 * Creamos una tipo de línea telefónica sin persistencia
	 * @param model modelo
	 * @return Vista
	 */
	...
	public String create(final Model model) {
		...
		// Creamos el registro
		model.addAttribute(ATTRIBUTE_TIPO, this.tipoLineaTelefonicaService.create());
	...
	/**
	 * Persistimos la tipo de línea telefónica pasado como parámetro
	...
	public String save(@Valid @ModelAttribute(ATTRIBUTE_TIPO) final TipoLineaTelefonicaDto tipoLineaTelefonicaDto, 
		...
		if (bindingResult.hasErrors()) {
			...
			this.tipoLineaTelefonicaService.save(tipoLineaTelefonicaDto);
			vista = ControllerConstants.REDIRECT.concat(MAP_READALL_TIPOS);
			log.info(LoggerConstants.LOG_SAVE, tipoLineaTelefonicaDto.getId());
		}
	...
	public String read(@PathVariable("id") final Short id, final Model model) {
		...
		// Contenido
		model.addAttribute(ATTRIBUTE_TIPO, this.tipoLineaTelefonicaService.read(id));
	...
	/**
	 * Preparamos la vista para la actulización del tipo de línea telefónica pasado como parámetro
	 * @param id Identificador
	 * @param model Modelo
	 * @return Vista
	 */
	...
	public String updatePOST(@Valid @ModelAttribute(ATTRIBUTE_TIPO) final TipoLineaTelefonicaDto tipoLineaTelefonicaDto, 
			final BindingResult bindingResult, final Model model) {		
		...
		if (bindingResult.hasErrors()) {			
			...
			this.tipoLineaTelefonicaService.update(tipoLineaTelefonicaDto);
			vista = ControllerConstants.REDIRECT.concat(MAP_READALL_TIPOS);
			log.info(LoggerConstants.LOG_UPDATE, tipoLineaTelefonicaDto.getId());			
		}
	...
	/**
	 * Preparamos la vista para la eliminación del tipo de línea telefónica pasado como parámetro
	 * @param id Identificador
	 * @param model Modelo
	 * @return Vista
	 */
	...
	public String deleteGET(@PathVariable("id") final Short id, final Model model) {
		...
		model.addAttribute(ControllerConstants.ATTRIBUTE_POPUP_ELIMINAR_PREGUNTA, 
				MessagesConstants.POPUP_ELIMINAR_TIPO_LINEATELEFONICA_PREGUNTA);
	...
	/**
	 * Eliminación del tipo de línea telefónica pasado como parámetro
	 * @param id Identificador
	 * @return Vista
	 */
	@PostMapping(MAP_DELETE_TIPO + "/{id}")
	public String deletePOST(@PathVariable("id") final Short id) {		
		this.tipoLineaTelefonicaService.delete(id);					
		log.info(LoggerConstants.LOG_DELETE);		
