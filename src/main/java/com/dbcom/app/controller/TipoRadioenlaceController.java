...
import com.dbcom.app.service.TipoRadioenlaceService;
...
@Slf4j
@Controller
public final class TipoRadioenlaceController {

	// Atributos de la vista
	private static final String ATTRIBUTE_TIPO = "tipoRadioenlace";

	// Vistas	
	private static final String VIEW_TIPO = ControllerConstants.MAP_PATH_MENU_TIPOS + ATTRIBUTE_TIPO;
	private static final String VIEW_TIPOS = ControllerConstants.MAP_PATH_MENU_TIPOS + "tiposRadioenlace";
	...
	private final TipoRadioenlaceService tipoRadioenlaceService;
	
	@Autowired
	public TipoRadioenlaceController(TipoRadioenlaceService tipoRadioenlaceService) {
		this.tipoRadioenlaceService = tipoRadioenlaceService;
	}
	...
	public String readAll(final Model model) {
		...
		// Contenido
		model.addAttribute(ControllerConstants.ATTRIBUTE_LISTA, this.tipoRadioenlaceService.readAll());		
	...
	/**
	 * Creamos una tipo de radioenlace sin persistencia
	 * @param model modelo
	 * @return Vista
	 */
	...
	public String create(final Model model) {
		...
		// Creamos el registro
		model.addAttribute(ATTRIBUTE_TIPO, this.tipoRadioenlaceService.create());
	...
	/**
	 * Persistimos la tipo de radioenlace pasado como parámetro
	...
	public String save(@Valid @ModelAttribute(ATTRIBUTE_TIPO) final TipoRadioenlaceDto tipoRadioenlaceDto, 
		...
		if (bindingResult.hasErrors()) {
			...
			this.tipoRadioenlaceService.save(tipoRadioenlaceDto);
			vista = ControllerConstants.REDIRECT.concat(MAP_READALL_TIPOS);
			log.info(LoggerConstants.LOG_SAVE, tipoRadioenlaceDto.getId());
		}
	...
	public String read(@PathVariable("id") final Short id, final Model model) {
		...
		// Contenido
		model.addAttribute(ATTRIBUTE_TIPO, this.tipoRadioenlaceService.read(id));
	...
	/**
	 * Preparamos la vista para la actulización del tipo de radioenlace pasado como parámetro
	 * @param id Identificador
	 * @param model Modelo
	 * @return Vista
	 */
	...
	public String updatePOST(@Valid @ModelAttribute(ATTRIBUTE_TIPO) final TipoRadioenlaceDto tipoRadioenlaceDto, 
			final BindingResult bindingResult, final Model model) {		
		...
		if (bindingResult.hasErrors()) {			
			...
			this.tipoRadioenlaceService.update(tipoRadioenlaceDto);
			vista = ControllerConstants.REDIRECT.concat(MAP_READALL_TIPOS);
			log.info(LoggerConstants.LOG_UPDATE, tipoRadioenlaceDto.getId());			
		}
	...
	/**
	 * Preparamos la vista para la eliminación del tipo de radioenlace pasado como parámetro
	 * @param id Identificador
	 * @param model Modelo
	 * @return Vista
	 */
	...
	public String deleteGET(@PathVariable("id") final Short id, final Model model) {
		...
		model.addAttribute(ControllerConstants.ATTRIBUTE_POPUP_ELIMINAR_PREGUNTA, 
				MessagesConstants.POPUP_ELIMINAR_TIPO_RADIOENLACE_PREGUNTA);
	...
	/**
	 * Eliminación del tipo de radioenlace pasado como parámetro
	 * @param id Identificador
	 * @return Vista
	 */
	@PostMapping(MAP_DELETE_TIPO + "/{id}")
	public String deletePOST(@PathVariable("id") final Short id) {		
		this.tipoRadioenlaceService.delete(id);					
		log.info(LoggerConstants.LOG_DELETE);		
