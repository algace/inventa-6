...
import com.dbcom.app.service.TipoModulacionService;
...
@Slf4j
@Controller
public final class TipoModulacionController {
	
	// Atributos de la vista
	private static final String ATTRIBUTE_TIPO = "tipoModulacion";

	// Vistas	
	private static final String VIEW_TIPO = ControllerConstants.MAP_PATH_MENU_TIPOS + ATTRIBUTE_TIPO;
	private static final String VIEW_TIPOS = ControllerConstants.MAP_PATH_MENU_TIPOS + "tiposModulacion";	
	...
	private final TipoModulacionService tipoModulacionService;
	
	@Autowired
	public TipoModulacionController(TipoModulacionService tipoModulacionService) {
		this.tipoModulacionService = tipoModulacionService;
	}
	...
	public String readAll(final Model model) {
		...
		// Contenido
		model.addAttribute(ControllerConstants.ATTRIBUTE_LISTA, this.tipoModulacionService.readAll());		
	...
	/**
	 * Creamos una tipo de modulación sin persistencia
	 * @param model modelo
	 * @return Vista
	 */
	...
	public String create(final Model model) {
		...
		// Creamos el registro
		model.addAttribute(ATTRIBUTE_TIPO, this.tipoModulacionService.create());
	...
	/**
	 * Persistimos la tipo de modulación pasado como parámetro
	...
	public String save(@Valid @ModelAttribute(ATTRIBUTE_TIPO) final TipoModulacionDto tipoModulacionDto, 
		...
		if (bindingResult.hasErrors()) {
			...
			this.tipoModulacionService.save(tipoModulacionDto);
			vista = ControllerConstants.REDIRECT.concat(MAP_READALL_TIPOS);
			log.info(LoggerConstants.LOG_SAVE, tipoModulacionDto.getId());
		}
	...
	public String read(@PathVariable("id") final Short id, final Model model) {
		...
		// Contenido
		model.addAttribute(ATTRIBUTE_TIPO, this.tipoModulacionService.read(id));
	...
	/**
	 * Preparamos la vista para la actulización del tipo de modulación pasado como parámetro
	 * @param id Identificador
	 * @param model Modelo
	 * @return Vista
	 */
	...
	public String updatePOST(@Valid @ModelAttribute(ATTRIBUTE_TIPO) final TipoModulacionDto tipoModulacionDto, 
			final BindingResult bindingResult, final Model model) {		
		...
		if (bindingResult.hasErrors()) {			
			...
			this.tipoModulacionService.update(tipoModulacionDto);
			vista = ControllerConstants.REDIRECT.concat(MAP_READALL_TIPOS);
			log.info(LoggerConstants.LOG_UPDATE, tipoModulacionDto.getId());			
		}
	...
	/**
	 * Preparamos la vista para la eliminación del tipo de modulación pasado como parámetro
	 * @param id Identificador
	 * @param model Modelo
	 * @return Vista
	 */
	...
	public String deleteGET(@PathVariable("id") final Short id, final Model model) {
		...
		model.addAttribute(ControllerConstants.ATTRIBUTE_POPUP_ELIMINAR_PREGUNTA, 
				MessagesConstants.POPUP_ELIMINAR_TIPO_MODULACION_PREGUNTA);
	...
	/**
	 * Eliminación del tipo de modulación pasado como parámetro
	 * @param id Identificador
	 * @return Vista
	 */
	@PostMapping(MAP_DELETE_TIPO + "/{id}")
	public String deletePOST(@PathVariable("id") final Short id) {		
		this.tipoModulacionService.delete(id);					
		log.info(LoggerConstants.LOG_DELETE);		
