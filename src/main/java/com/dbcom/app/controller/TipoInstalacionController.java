...
import com.dbcom.app.service.TipoInstalacionService;
...
@Slf4j
@Controller
public final class TipoInstalacionController {

	// Atributos de la vista
	private static final String ATTRIBUTE_TIPO = "tipoInstalacion";

	// Vistas	
	private static final String VIEW_TIPO = ControllerConstants.MAP_PATH_MENU_TIPOS + ATTRIBUTE_TIPO;
	private static final String VIEW_TIPOS = ControllerConstants.MAP_PATH_MENU_TIPOS + "tiposInstalacion";
	...
	private final TipoInstalacionService tipoInstalacionService;
	
	@Autowired
	public TipoInstalacionController(TipoInstalacionService tipoInstalacionService) {
		this.tipoInstalacionService = tipoInstalacionService;
	}
	...
	public String readAll(final Model model) {
		...
		// Contenido
		model.addAttribute(ControllerConstants.ATTRIBUTE_LISTA, this.tipoInstalacionService.readAll());		
	...
	/**
	 * Creamos una tipo de instalación sin persistencia
	 * @param model modelo
	 * @return Vista
	 */
	...
	public String create(final Model model) {
		...
		// Creamos el registro
		model.addAttribute(ATTRIBUTE_TIPO, this.tipoInstalacionService.create());
	...
	/**
	 * Persistimos la tipo de instalación pasado como parámetro
	...
	public String save(@Valid @ModelAttribute(ATTRIBUTE_TIPO) final TipoInstalacionDto tipoInstalacionDto, 
		...
		if (bindingResult.hasErrors()) {
			...
			this.tipoInstalacionService.save(tipoInstalacionDto);
			vista = ControllerConstants.REDIRECT.concat(MAP_READALL_TIPOS);
			log.info(LoggerConstants.LOG_SAVE, tipoInstalacionDto.getId());
		}
	...
	public String read(@PathVariable("id") final Short id, final Model model) {
		...
		// Contenido
		model.addAttribute(ATTRIBUTE_TIPO, this.tipoInstalacionService.read(id));
	...
	/**
	 * Preparamos la vista para la actulización del tipo de instalación pasado como parámetro
	 * @param id Identificador
	 * @param model Modelo
	 * @return Vista
	 */
	...
	public String updatePOST(@Valid @ModelAttribute(ATTRIBUTE_TIPO) final TipoInstalacionDto tipoInstalacionDto, 
			final BindingResult bindingResult, final Model model) {		
		...
		if (bindingResult.hasErrors()) {			
			...
			this.tipoInstalacionService.update(tipoInstalacionDto);
			vista = ControllerConstants.REDIRECT.concat(MAP_READALL_TIPOS);
			log.info(LoggerConstants.LOG_UPDATE, tipoInstalacionDto.getId());			
		}
	...
	/**
	 * Preparamos la vista para la eliminación del tipo de instalación pasado como parámetro
	 * @param id Identificador
	 * @param model Modelo
	 * @return Vista
	 */
	...
	public String deleteGET(@PathVariable("id") final Short id, final Model model) {
		...
		model.addAttribute(ControllerConstants.ATTRIBUTE_POPUP_ELIMINAR_PREGUNTA, 
				MessagesConstants.POPUP_ELIMINAR_TIPO_INSTALACION_PREGUNTA);
	...
	/**
	 * Eliminación del tipo de instalación pasado como parámetro
	 * @param id Identificador
	 * @return Vista
	 */
	@PostMapping(MAP_DELETE_TIPO + "/{id}")
	public String deletePOST(@PathVariable("id") final Short id) {		
		this.tipoInstalacionService.delete(id);					
		log.info(LoggerConstants.LOG_DELETE);		
