...
import com.dbcom.app.service.TipoConfiguracionTiradaService;
...
@Slf4j
@Controller
public final class TipoConfiguracionTiradaController {

	// Atributos de la vista
	private static final String ATTRIBUTE_TIPO = "tipoConfiguracionTirada";

	// Vistas	
	private static final String VIEW_TIPO = ControllerConstants.MAP_PATH_MENU_TIPOS + ATTRIBUTE_TIPO;
	private static final String VIEW_TIPOS = ControllerConstants.MAP_PATH_MENU_TIPOS + "tiposConfiguracionTirada";
	...
	private final TipoConfiguracionTiradaService tipoConfiguracionTiradaService;
	
	@Autowired
	public TipoConfiguracionTiradaController(TipoConfiguracionTiradaService tipoConfiguracionTiradaService) {
		this.tipoConfiguracionTiradaService = tipoConfiguracionTiradaService;
	}
	...
	public String readAll(final Model model) {
		...
		// Contenido
		model.addAttribute(ControllerConstants.ATTRIBUTE_LISTA, this.tipoConfiguracionTiradaService.readAll());		
	...
	/**
	 * Creamos una tipo de configuración tirada sin persistencia
	 * @param model modelo
	 * @return Vista
	 */
	...
	public String create(final Model model) {
		...
		// Creamos el registro
		model.addAttribute(ATTRIBUTE_TIPO, this.tipoConfiguracionTiradaService.create());
	...
	/**
	 * Persistimos la tipo de configuración tirada pasado como parámetro
	...
	public String save(@Valid @ModelAttribute(ATTRIBUTE_TIPO) final TipoConfiguracionTiradaDto tipoConfiguracionTiradaDto, 
		...
		if (bindingResult.hasErrors()) {
			...
			this.tipoConfiguracionTiradaService.save(tipoConfiguracionTiradaDto);
			vista = ControllerConstants.REDIRECT.concat(MAP_READALL_TIPOS);
			log.info(LoggerConstants.LOG_SAVE, tipoConfiguracionTiradaDto.getId());
		}
	...
	public String read(@PathVariable("id") final Short id, final Model model) {
		...
		// Contenido
		model.addAttribute(ATTRIBUTE_TIPO, this.tipoConfiguracionTiradaService.read(id));
	...
	/**
	 * Preparamos la vista para la actulización del tipo de configuración tirada pasado como parámetro
	 * @param id Identificador
	 * @param model Modelo
	 * @return Vista
	 */
	...
	/**
	 * Actualizamos el tipo de configuración tirada pasado como parámetro
	 * @param tipoConfiguracionTiradaDto Tipo de configuración tirada a actualizar
	 * @param bindingResult Validaciones
	 * @param model Modelo
	 * @return Vista
	 */
	@PostMapping(MAP_UPDATE_TIPO)
	public String updatePOST(@Valid @ModelAttribute(ATTRIBUTE_TIPO) final TipoConfiguracionTiradaDto tipoConfiguracionTiradaDto, 
			final BindingResult bindingResult, final Model model) {		
		...
		if (bindingResult.hasErrors()) {			
			...
			this.tipoConfiguracionTiradaService.update(tipoConfiguracionTiradaDto);
			vista = ControllerConstants.REDIRECT.concat(MAP_READALL_TIPOS);
			log.info(LoggerConstants.LOG_UPDATE, tipoConfiguracionTiradaDto.getId());			
		}
	...
	/**
	 * Preparamos la vista para la eliminación del tipo de configuración tirada pasado como parámetro
	 * @param id Identificador
	 * @param model Modelo
	 * @return Vista
	 */
	...
	public String deleteGET(@PathVariable("id") final Short id, final Model model) {
		...
		model.addAttribute(ControllerConstants.ATTRIBUTE_POPUP_ELIMINAR_PREGUNTA, 
				MessagesConstants.POPUP_ELIMINAR_TIPO_CONFICURACIONTIRADA_PREGUNTA);
	...
	/**
	 * Eliminación del tipo de configuración tirada pasado como parámetro
	 * @param id Identificador
	 * @return Vista
	 */
	@PostMapping(MAP_DELETE_TIPO + "/{id}")
	public String deletePOST(@PathVariable("id") final Short id) {		
		this.tipoConfiguracionTiradaService.delete(id);					
		log.info(LoggerConstants.LOG_DELETE);		
