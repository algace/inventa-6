...
import com.dbcom.app.service.TipoCapacidadEnlaceTTService;
...
@Slf4j
@Controller
public final class TipoCapacidadEnlaceTTController {

	// Atributos de la vista
	private static final String ATTRIBUTE_TIPO = "tipoCapacidadEnlaceTT";

	// Vistas	
	private static final String VIEW_TIPO = ControllerConstants.MAP_PATH_MENU_TIPOS + ATTRIBUTE_TIPO;
	private static final String VIEW_TIPOS = ControllerConstants.MAP_PATH_MENU_TIPOS + "tiposCapacidadEnlaceTT";
	...
	private final TipoCapacidadEnlaceTTService tipoCapacidadEnlaceTTService;
	
	@Autowired
	public TipoCapacidadEnlaceTTController(TipoCapacidadEnlaceTTService tipoCapacidadEnlaceTTService) {
		this.tipoCapacidadEnlaceTTService = tipoCapacidadEnlaceTTService;
	}
	...
	public String readAll(final Model model) {
		...
		// Contenido
		model.addAttribute(ControllerConstants.ATTRIBUTE_LISTA, this.tipoCapacidadEnlaceTTService.readAll());		
	...
	/**
	 * Creamos una tipo de capacidad enlace TT sin persistencia
	 * @param model modelo
	 * @return Vista
	 */
	...
	public String create(final Model model) {
		...
		// Creamos el registro
		model.addAttribute(ATTRIBUTE_TIPO, this.tipoCapacidadEnlaceTTService.create());
	...
	/**
	 * Persistimos la tipo de capacidad enlace TT pasado como parámetro
	...
	public String save(@Valid @ModelAttribute(ATTRIBUTE_TIPO) final TipoCapacidadEnlaceTTDto tipoCapacidadEnlaceTTDto, 
		...
		if (bindingResult.hasErrors()) {
			...
			this.tipoCapacidadEnlaceTTService.save(tipoCapacidadEnlaceTTDto);
			vista = ControllerConstants.REDIRECT.concat(MAP_READALL_TIPOS);
			log.info(LoggerConstants.LOG_SAVE, tipoCapacidadEnlaceTTDto.getId());
		}
	...
	public String read(@PathVariable("id") final Short id, final Model model) {
		...
		// Contenido
		model.addAttribute(ATTRIBUTE_TIPO, this.tipoCapacidadEnlaceTTService.read(id));
	...
	/**
	 * Preparamos la vista para la actulización del tipo de capacidad enlace TT pasado como parámetro
	 * @param id Identificador
	 * @param model Modelo
	 * @return Vista
	 */
	...
	public String updatePOST(@Valid @ModelAttribute(ATTRIBUTE_TIPO) final TipoCapacidadEnlaceTTDto tipoCapacidadEnlaceTTDto, 
			final BindingResult bindingResult, final Model model) {		
		...
		if (bindingResult.hasErrors()) {			
			...
			this.tipoCapacidadEnlaceTTService.update(tipoCapacidadEnlaceTTDto);
			vista = ControllerConstants.REDIRECT.concat(MAP_READALL_TIPOS);
			log.info(LoggerConstants.LOG_UPDATE, tipoCapacidadEnlaceTTDto.getId());			
		}
	...
	/**
	 * Preparamos la vista para la eliminación del tipo de capacidad enlace TT pasado como parámetro
	 * @param id Identificador
	 * @param model Modelo
	 * @return Vista
	 */
	...
	public String deleteGET(@PathVariable("id") final Short id, final Model model) {
		...
		model.addAttribute(ControllerConstants.ATTRIBUTE_POPUP_ELIMINAR_PREGUNTA, 
				MessagesConstants.POPUP_ELIMINAR_TIPO_CAPACIDADENLACETT_PREGUNTA);
	...
	/**
	 * Eliminación del tipo de capacidad enlace TT pasado como parámetro
	 * @param id Identificador
	 * @return Vista
	 */
	@PostMapping(MAP_DELETE_TIPO + "/{id}")
	public String deletePOST(@PathVariable("id") final Short id) {		
		this.tipoCapacidadEnlaceTTService.delete(id);					
		log.info(LoggerConstants.LOG_DELETE);		
